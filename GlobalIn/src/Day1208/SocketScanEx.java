...
public class SocketScanEx extends JFrame implements ActionListener, Runnable {
	...
	public void run() {
		...
		for (int i = 1; i < 65536; i++) {
			try {
				// 소켓 객체가 생성되면 호스와 연결돼 해당 포스트의 포트를 사용하고 있다는 의미
				s = new Socket(ip, i);
				// 포트가 사용 중이라면 list에 추가
				listmodel.insertElementAt(i + "번 포트 번호가 사용 중입니다.", index);
				jlist_result.setSelectedIndex(index);
			} catch (IOException io) { // 포트를 사용하고 있지 않다면 예외 발생
